...
for i in range(384 // divisor):
    try:
        dataset.append([tv_tensors.Image(PIL.Image.open("./data/normal/" + str(i) + ".png")), 0])
    except:
        continue
for i in range(1999 // divisor):
    try:
        dataset.append([tv_tensors.Image(PIL.Image.open("./data/fracture/" + str(i) + ".png")), 1])
    except:
        continue
...
while num_normal < num_broken:
    r_num = random.randint(0, len_training)
    if(training[r_num][1] == 0):
...
class BaseLine_Static(nn.Module):
    ...
    def forward(self, x):
        output = torch.ones((1,), dtype=torch.float32)
...
torch.save(model_augmented.state_dict(), './augmented_model.pt')
print('Augmented Model Saved')
torch.save(model_downsized.state_dict(), './downsized_model.pt')
